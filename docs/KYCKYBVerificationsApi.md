...
func main() {
    verification := *openapiclient.NewVerification(openapiclient.verification_result("ACCEPTED"), time.Now(), openapiclient.verification_type1("DOCUMENT_VERIFICATION")) // Verification | Verification result to create.
...
> VerificationList ListVerifications1(ctx).VerificationType(verificationType).BusinessId(businessId).IncludeHistory(includeHistory).Id(id).PageToken(pageToken).Result(result).PersonId(personId).Limit(limit).Execute()
...
func main() {
    verificationType := []openapiclient.VerificationType1{openapiclient.verification_type1("DOCUMENT_VERIFICATION")} // []VerificationType1 | The type of verification run. Multiple values can be provided as a comma-separated list.  (optional)
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.KYCKYBVerificationsAPI.ListVerifications1(context.Background()).VerificationType(verificationType).BusinessId(businessId).IncludeHistory(includeHistory).Id(id).PageToken(pageToken).Result(result).PersonId(personId).Limit(limit).Execute()
    if err != nil {
