...
> InternalAccountsList ListInternalAccounts(ctx).PageToken(pageToken).Limit(limit).Id(id).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.InternalAccountsAPI.ListInternalAccounts(context.Background()).PageToken(pageToken).Limit(limit).Id(id).Execute()
    if err != nil {
