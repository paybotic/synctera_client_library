...
func main() {
    noteCreate := *openapiclient.NewNoteCreate("Customer was frozen to investigate fraud.", "7d943c51-e4ff-4e57-9558-08cab6b963c7", openapiclient.related_resource_type1("ACCOUNT")) // NoteCreate | note to create
...
> NoteList ListNotes(ctx).RelatedResourceId(relatedResourceId).AuthorUserId(authorUserId).Id(id).RelatedResourceType(relatedResourceType).PageToken(pageToken).Limit(limit).Tenant(tenant).Execute()
...
func main() {
    ...
    relatedResourceType := openapiclient.related_resource_type1("ACCOUNT") // RelatedResourceType1 | Only return notes that have a related resource with the specified type. (optional)
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.NotesAPI.ListNotes(context.Background()).RelatedResourceId(relatedResourceId).AuthorUserId(authorUserId).Id(id).RelatedResourceType(relatedResourceType).PageToken(pageToken).Limit(limit).Tenant(tenant).Execute()
    if err != nil {
 ...
 **relatedResourceType** | [**RelatedResourceType1**](RelatedResourceType1.md) | Only return notes that have a related resource with the specified type. | 
 **pageToken** | **string** |  | 
