...
[**RevealPersonalId**](PersonsAPI.md#RevealPersonalId) | **Get** /persons/personal_ids/{personal_id_id}/reveal | Reveal Personal ID
...
func main() {
	postPersonalIdWCust := *openapiclient.NewPostPersonalIdWCust(openapiclient.personal_id_type("ITIN"), "123-45-6789", "21943c51-e4ff-4e57-9558-08cab6b963fe") // PostPersonalIdWCust | 
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)
...
> PersonList ListPersons(ctx).Uuid(uuid).FirstName(firstName).LastName(lastName).Dob(dob).PhoneNumber(phoneNumber).Email(email).SsnLast4(ssnLast4).BanStatus(banStatus).Status(status).VerificationStatus(verificationStatus).IsCustomer(isCustomer).Limit(limit).PageToken(pageToken).SortBy(sortBy).HasAccounts(hasAccounts).IncludeConsoleUsers(includeConsoleUsers).Classifications(classifications).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.PersonsAPI.ListPersons(context.Background()).Uuid(uuid).FirstName(firstName).LastName(lastName).Dob(dob).PhoneNumber(phoneNumber).Email(email).SsnLast4(ssnLast4).BanStatus(banStatus).Status(status).VerificationStatus(verificationStatus).IsCustomer(isCustomer).Limit(limit).PageToken(pageToken).SortBy(sortBy).HasAccounts(hasAccounts).IncludeConsoleUsers(includeConsoleUsers).Classifications(classifications).Execute()
	if err != nil {
 ...
 **email** | **string** |  | 
 **ssnLast4** | **string** |  | 
 **banStatus** | [**[]BanStatus**](BanStatus.md) | Ban status of the person. Multiple values can be provided as a comma-seperated list of the following: * &#x60;ALLOWED&#x60; – person is not banned or suspended * &#x60;SUSPENDED&#x60; - person is suspended * &#x60;BANNED&#x60; – person is banned  | 
 **status** | [**PersonStatus**](PersonStatus.md) |  | 
 **verificationStatus** | [**[]VerificationStatus**](VerificationStatus.md) | Verification status of the person. Multiple values can be provided as a comma-seperated list of the following: * &#x60;UNVERIFIED&#x60; – verification has not been completed for this customer. * &#x60;PENDING&#x60; – verification is in progress for this customer. * &#x60;PROVISIONAL&#x60; – partially verified or verified with restrictions. * &#x60;ACCEPTED&#x60; – the customer has been verified. * &#x60;REVIEW&#x60; – verification has run and issues have been identified and require review. * &#x60;REJECTED&#x60; – the customer was rejected and should not be allowed to take certain actions e.g., open an account.  | 
...
> PersonalIdRevealResponse RevealPersonalId(ctx, personalIdId).PersonalIdConfigurationId(personalIdConfigurationId).Execute()
...
func main() {
	personalIdId := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | UUID for a personal identifier.
	personalIdConfigurationId := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | UUID of the personal ID configuration to use for encryption. If not provided, the oldest configured public key will be used.  (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.PersonsAPI.RevealPersonalId(context.Background(), personalIdId).PersonalIdConfigurationId(personalIdConfigurationId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `PersonsAPI.RevealPersonalId``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `RevealPersonalId`: PersonalIdRevealResponse
	fmt.Fprintf(os.Stdout, "Response from `PersonsAPI.RevealPersonalId`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiRevealPersonalIdRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------

 **personalIdConfigurationId** | **string** | UUID of the personal ID configuration to use for encryption. If not provided, the oldest configured public key will be used.  | 

### Return type

[**PersonalIdRevealResponse**](PersonalIdRevealResponse.md)
