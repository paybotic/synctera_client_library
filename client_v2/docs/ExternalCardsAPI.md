# \ExternalCardsAPI

All URIs are relative to *https://api.synctera.com/v2*

Method | HTTP request | Description
------------- | ------------- | -------------
[**Authenticate3DS**](ExternalCardsAPI.md#Authenticate3DS) | **Post** /external_cards/authenticate_3ds | Authenticate 3DS
[**CreateApplePayExternalCardTransfer**](ExternalCardsAPI.md#CreateApplePayExternalCardTransfer) | **Post** /external_cards/transfers/applepay | Create Apple Pay External Card Transfer
[**CreateExternalCard**](ExternalCardsAPI.md#CreateExternalCard) | **Post** /external_cards | Create External Card
[**CreateExternalCardFromToken**](ExternalCardsAPI.md#CreateExternalCardFromToken) | **Post** /external_cards/tokens | Create External Card from token
[**CreateExternalCardTransfer**](ExternalCardsAPI.md#CreateExternalCardTransfer) | **Post** /external_cards/transfers | Create External Card Transfer
[**CreateExternalCardTransferReversal**](ExternalCardsAPI.md#CreateExternalCardTransferReversal) | **Post** /external_cards/transfers/{transfer_id}/reversals | Create External Card Transfer Reversal
[**CreateGooglePayExternalCardTransfer**](ExternalCardsAPI.md#CreateGooglePayExternalCardTransfer) | **Post** /external_cards/transfers/googlepay | Create Google Pay External Card Transfer
[**DeleteExternalCard**](ExternalCardsAPI.md#DeleteExternalCard) | **Delete** /external_cards/{external_card_id} | Delete External Card
[**GetExternalCard**](ExternalCardsAPI.md#GetExternalCard) | **Get** /external_cards/{external_card_id} | Get External Card
[**GetExternalCardTransfer**](ExternalCardsAPI.md#GetExternalCardTransfer) | **Get** /external_cards/transfers/{transfer_id} | Get External Card Transfer
[**GetExternalCardWidgetAccessToken**](ExternalCardsAPI.md#GetExternalCardWidgetAccessToken) | **Get** /external_cards/widget_token | Get an external card widget access token
[**Initialize3DS**](ExternalCardsAPI.md#Initialize3DS) | **Post** /external_cards/initialize_3ds | Initialize 3DS
[**ListExternalCardTransfers**](ExternalCardsAPI.md#ListExternalCardTransfers) | **Get** /external_cards/transfers | List External Transfers
[**ListExternalCards**](ExternalCardsAPI.md#ListExternalCards) | **Get** /external_cards | List External Cards
[**Lookup3DS**](ExternalCardsAPI.md#Lookup3DS) | **Post** /external_cards/lookup_3ds | Lookup 3DS
[**RevealExternalCardDetails**](ExternalCardsAPI.md#RevealExternalCardDetails) | **Get** /external_cards/{external_card_id}/reveal | Reveal External Card Details
[**UpdateExternalCard**](ExternalCardsAPI.md#UpdateExternalCard) | **Patch** /external_cards/{external_card_id} | Update External Card



## Authenticate3DS

> Authenticate3dsResponse Authenticate3DS(ctx).Authenticate3dsRequest(authenticate3dsRequest).IdempotencyKey(idempotencyKey).Execute()

Authenticate 3DS



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	authenticate3dsRequest := *openapiclient.NewAuthenticate3dsRequest("ChallengeJwt_example", "7d943c51-e4ff-4e57-9558-08cab6b963c7") // Authenticate3dsRequest | Details of the External Card Transfer 3DS authentication challenge validation.
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.Authenticate3DS(context.Background()).Authenticate3dsRequest(authenticate3dsRequest).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.Authenticate3DS``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `Authenticate3DS`: Authenticate3dsResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.Authenticate3DS`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiAuthenticate3DSRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **authenticate3dsRequest** | [**Authenticate3dsRequest**](Authenticate3dsRequest.md) | Details of the External Card Transfer 3DS authentication challenge validation. | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**Authenticate3dsResponse**](Authenticate3dsResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## CreateApplePayExternalCardTransfer

> CreateTransferResponseApplePay CreateApplePayExternalCardTransfer(ctx).TransferRequestApplePay(transferRequestApplePay).IdempotencyKey(idempotencyKey).Execute()

Create Apple Pay External Card Transfer



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	transferRequestApplePay := *openapiclient.NewTransferRequestApplePay(int32(123), "7d943c51-e4ff-4e57-9558-08cab6b963c7", *openapiclient.NewApplePayPayment(*openapiclient.NewApplePayPaymentToken(*openapiclient.NewApplePayPaymentData("Data_example", *openapiclient.NewApplePayPaymentHeader("EphemeralPublicKey_example", "PublicKeyHash_example", "TransactionId_example"), "Signature_example", "Version_example"), *openapiclient.NewApplePayPaymentMethod(), "TransactionIdentifier_example")), "7d943c51-e4ff-4e57-9558-08cab6b963c7") // TransferRequestApplePay | Details of the Apple Pay External Card Transfer to create
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.CreateApplePayExternalCardTransfer(context.Background()).TransferRequestApplePay(transferRequestApplePay).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.CreateApplePayExternalCardTransfer``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CreateApplePayExternalCardTransfer`: CreateTransferResponseApplePay
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.CreateApplePayExternalCardTransfer`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiCreateApplePayExternalCardTransferRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **transferRequestApplePay** | [**TransferRequestApplePay**](TransferRequestApplePay.md) | Details of the Apple Pay External Card Transfer to create | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**CreateTransferResponseApplePay**](CreateTransferResponseApplePay.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## CreateExternalCard

> ExternalCardResponse CreateExternalCard(ctx).ExternalCardRequestPan(externalCardRequestPan).IdempotencyKey(idempotencyKey).Execute()

Create External Card



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	externalCardRequestPan := *openapiclient.NewExternalCardRequestPan("7d943c51-e4ff-4e57-9558-08cab6b963c7", "ExpirationMonth_example", "ExpirationYear_example", "Jean Valjean", "Pan_example") // ExternalCardRequestPan | Details of the External Card to create
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.CreateExternalCard(context.Background()).ExternalCardRequestPan(externalCardRequestPan).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.CreateExternalCard``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CreateExternalCard`: ExternalCardResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.CreateExternalCard`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiCreateExternalCardRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **externalCardRequestPan** | [**ExternalCardRequestPan**](ExternalCardRequestPan.md) | Details of the External Card to create | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**ExternalCardResponse**](ExternalCardResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## CreateExternalCardFromToken

> ExternalCardResponse CreateExternalCardFromToken(ctx).ExternalCardRequest(externalCardRequest).IdempotencyKey(idempotencyKey).Execute()

Create External Card from token



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	externalCardRequest := *openapiclient.NewExternalCardRequest("7d943c51-e4ff-4e57-9558-08cab6b963c7", "Jean Valjean", "Token_example") // ExternalCardRequest | Details of the External Card to create
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.CreateExternalCardFromToken(context.Background()).ExternalCardRequest(externalCardRequest).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.CreateExternalCardFromToken``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CreateExternalCardFromToken`: ExternalCardResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.CreateExternalCardFromToken`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiCreateExternalCardFromTokenRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **externalCardRequest** | [**ExternalCardRequest**](ExternalCardRequest.md) | Details of the External Card to create | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**ExternalCardResponse**](ExternalCardResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## CreateExternalCardTransfer

> CreateTransferResponse CreateExternalCardTransfer(ctx).TransferRequest(transferRequest).IdempotencyKey(idempotencyKey).Execute()

Create External Card Transfer



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	transferRequest := openapiclient.transfer_request{TransferRequestPull: openapiclient.NewTransferRequestPull(int32(123), "7d943c51-e4ff-4e57-9558-08cab6b963c7", "7d943c51-e4ff-4e57-9558-08cab6b963c7", openapiclient.create_transfer_type("PUSH"))} // TransferRequest | Details of the External Card Transfer to create
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.CreateExternalCardTransfer(context.Background()).TransferRequest(transferRequest).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.CreateExternalCardTransfer``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CreateExternalCardTransfer`: CreateTransferResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.CreateExternalCardTransfer`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiCreateExternalCardTransferRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **transferRequest** | [**TransferRequest**](TransferRequest.md) | Details of the External Card Transfer to create | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**CreateTransferResponse**](CreateTransferResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## CreateExternalCardTransferReversal

> TransferResponse CreateExternalCardTransferReversal(ctx, transferId).TransferReversalRequest(transferReversalRequest).IdempotencyKey(idempotencyKey).Execute()

Create External Card Transfer Reversal



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	transferId := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | The unique identifier of a transfer
	transferReversalRequest := *openapiclient.NewTransferReversalRequest(int32(123), "USD") // TransferReversalRequest | Details of the External Card Transfer Reversal to create
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.CreateExternalCardTransferReversal(context.Background(), transferId).TransferReversalRequest(transferReversalRequest).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.CreateExternalCardTransferReversal``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CreateExternalCardTransferReversal`: TransferResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.CreateExternalCardTransferReversal`: %v\n", resp)
}
```

### Path Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
**ctx** | **context.Context** | context for authentication, logging, cancellation, deadlines, tracing, etc.
**transferId** | **string** | The unique identifier of a transfer | 

### Other Parameters

Other parameters are passed through a pointer to a apiCreateExternalCardTransferReversalRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------

 **transferReversalRequest** | [**TransferReversalRequest**](TransferReversalRequest.md) | Details of the External Card Transfer Reversal to create | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**TransferResponse**](TransferResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## CreateGooglePayExternalCardTransfer

> CreateTransferResponseGooglePay CreateGooglePayExternalCardTransfer(ctx).TransferRequestGooglePay(transferRequestGooglePay).IdempotencyKey(idempotencyKey).Execute()

Create Google Pay External Card Transfer



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	transferRequestGooglePay := *openapiclient.NewTransferRequestGooglePay(int32(123), "7d943c51-e4ff-4e57-9558-08cab6b963c7", "7d943c51-e4ff-4e57-9558-08cab6b963c7", *openapiclient.NewGooglePayPaymentData(int32(123), int32(123), *openapiclient.NewGooglePayPaymentMethodData("Description_example", *openapiclient.NewGooglePayInfo(*openapiclient.NewGooglePayAssuranceDetails(), "CardDetails_example", "CardNetwork_example"), *openapiclient.NewGooglePayTokenizationData("Token_example", "Type_example"), "Type_example"))) // TransferRequestGooglePay | Details of the Google Pay External Card Transfer to create
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.CreateGooglePayExternalCardTransfer(context.Background()).TransferRequestGooglePay(transferRequestGooglePay).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.CreateGooglePayExternalCardTransfer``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CreateGooglePayExternalCardTransfer`: CreateTransferResponseGooglePay
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.CreateGooglePayExternalCardTransfer`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiCreateGooglePayExternalCardTransferRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **transferRequestGooglePay** | [**TransferRequestGooglePay**](TransferRequestGooglePay.md) | Details of the Google Pay External Card Transfer to create | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**CreateTransferResponseGooglePay**](CreateTransferResponseGooglePay.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## DeleteExternalCard

> DeleteResponse DeleteExternalCard(ctx, externalCardId).Execute()

Delete External Card



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	externalCardId := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | The unique identifier of an external card

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.DeleteExternalCard(context.Background(), externalCardId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.DeleteExternalCard``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `DeleteExternalCard`: DeleteResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.DeleteExternalCard`: %v\n", resp)
}
```

### Path Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
**ctx** | **context.Context** | context for authentication, logging, cancellation, deadlines, tracing, etc.
**externalCardId** | **string** | The unique identifier of an external card | 

### Other Parameters

Other parameters are passed through a pointer to a apiDeleteExternalCardRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------


### Return type

[**DeleteResponse**](DeleteResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## GetExternalCard

> ExternalCardResponse GetExternalCard(ctx, externalCardId).Execute()

Get External Card



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	externalCardId := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | The unique identifier of an external card

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.GetExternalCard(context.Background(), externalCardId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.GetExternalCard``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetExternalCard`: ExternalCardResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.GetExternalCard`: %v\n", resp)
}
```

### Path Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
**ctx** | **context.Context** | context for authentication, logging, cancellation, deadlines, tracing, etc.
**externalCardId** | **string** | The unique identifier of an external card | 

### Other Parameters

Other parameters are passed through a pointer to a apiGetExternalCardRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------


### Return type

[**ExternalCardResponse**](ExternalCardResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## GetExternalCardTransfer

> TransferResponse GetExternalCardTransfer(ctx, transferId).Execute()

Get External Card Transfer



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	transferId := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | The unique identifier of a transfer

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.GetExternalCardTransfer(context.Background(), transferId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.GetExternalCardTransfer``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetExternalCardTransfer`: TransferResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.GetExternalCardTransfer`: %v\n", resp)
}
```

### Path Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
**ctx** | **context.Context** | context for authentication, logging, cancellation, deadlines, tracing, etc.
**transferId** | **string** | The unique identifier of a transfer | 

### Other Parameters

Other parameters are passed through a pointer to a apiGetExternalCardTransferRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------


### Return type

[**TransferResponse**](TransferResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## GetExternalCardWidgetAccessToken

> WidgetToken GetExternalCardWidgetAccessToken(ctx).WidgetType(widgetType).CustomerId(customerId).BusinessId(businessId).ExternalCardId(externalCardId).IdempotencyKey(idempotencyKey).Execute()

Get an external card widget access token



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	widgetType := openapiclient.external_card_widget_type("TOKENIZE") // ExternalCardWidgetType | 
	customerId := []string{"Inner_example"} // []string | Return the credit scores associated with the specified customer. Multiple IDs can be provided as a comma-separated list.  (optional)
	businessId := "38400000-8cf0-11bd-b23e-10b96e4ef00d" // string |  (optional)
	externalCardId := "38400000-8cf0-11bd-b23e-10b96e4ef00d" // string |  (optional)
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.GetExternalCardWidgetAccessToken(context.Background()).WidgetType(widgetType).CustomerId(customerId).BusinessId(businessId).ExternalCardId(externalCardId).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.GetExternalCardWidgetAccessToken``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetExternalCardWidgetAccessToken`: WidgetToken
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.GetExternalCardWidgetAccessToken`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiGetExternalCardWidgetAccessTokenRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **widgetType** | [**ExternalCardWidgetType**](ExternalCardWidgetType.md) |  | 
 **customerId** | **[]string** | Return the credit scores associated with the specified customer. Multiple IDs can be provided as a comma-separated list.  | 
 **businessId** | **string** |  | 
 **externalCardId** | **string** |  | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**WidgetToken**](WidgetToken.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## Initialize3DS

> Initialize3dsResponse Initialize3DS(ctx).Initialize3dsRequest(initialize3dsRequest).IdempotencyKey(idempotencyKey).Execute()

Initialize 3DS



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	initialize3dsRequest := *openapiclient.NewInitialize3dsRequest(int32(123), "USD", "7d943c51-e4ff-4e57-9558-08cab6b963c7") // Initialize3dsRequest | Details of the External Card Transfer 3DS authentication to initialize.
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.Initialize3DS(context.Background()).Initialize3dsRequest(initialize3dsRequest).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.Initialize3DS``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `Initialize3DS`: Initialize3dsResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.Initialize3DS`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiInitialize3DSRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **initialize3dsRequest** | [**Initialize3dsRequest**](Initialize3dsRequest.md) | Details of the External Card Transfer 3DS authentication to initialize. | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**Initialize3dsResponse**](Initialize3dsResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## ListExternalCardTransfers

> TransferListResponse ListExternalCardTransfers(ctx).Limit(limit).PageToken(pageToken).AccountId(accountId).Amount(amount).CustomerId(customerId).ExternalCardId(externalCardId).FromAmount(fromAmount).FromCreationTime(fromCreationTime).FromLastUpdatedTime(fromLastUpdatedTime).Id(id).OriginalTransferId(originalTransferId).OriginatingAccountId(originatingAccountId).OriginatingCustomerId(originatingCustomerId).PaymentAccountReference(paymentAccountReference).Status(status).Tenant(tenant).ThreeDsId(threeDsId).ToAmount(toAmount).ToCreationTime(toCreationTime).ToLastUpdatedTime(toLastUpdatedTime).TransactionId(transactionId).Type_(type_).Execute()

List External Transfers



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
    "time"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	limit := int32(100) // int32 |  (optional) (default to 100)
	pageToken := "a8937a0d" // string |  (optional)
	accountId := []string{"Inner_example"} // []string |  (optional)
	amount := []int32{int32(123)} // []int32 |  (optional)
	customerId := []string{"Inner_example"} // []string |  (optional)
	externalCardId := []string{"Inner_example"} // []string |  (optional)
	fromAmount := int32(56) // int32 |  (optional)
	fromCreationTime := time.Now() // time.Time |  (optional)
	fromLastUpdatedTime := time.Now() // time.Time |  (optional)
	id := []string{"Inner_example"} // []string |  (optional)
	originalTransferId := []string{"Inner_example"} // []string |  (optional)
	originatingAccountId := []string{"Inner_example"} // []string |  (optional)
	originatingCustomerId := []string{"Inner_example"} // []string |  (optional)
	paymentAccountReference := "paymentAccountReference_example" // string |  (optional)
	status := []openapiclient.TransferStatus{openapiclient.transfer_status("SUCCEEDED")} // []TransferStatus |  (optional)
	tenant := []string{"abcdef_ghijkl"} // []string |  (optional)
	threeDsId := []string{"Inner_example"} // []string |  (optional)
	toAmount := int32(56) // int32 |  (optional)
	toCreationTime := time.Now() // time.Time |  (optional)
	toLastUpdatedTime := time.Now() // time.Time |  (optional)
	transactionId := []string{"Inner_example"} // []string |  (optional)
	type_ := []openapiclient.TransferType{openapiclient.transfer_type("PUSH")} // []TransferType |  (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.ListExternalCardTransfers(context.Background()).Limit(limit).PageToken(pageToken).AccountId(accountId).Amount(amount).CustomerId(customerId).ExternalCardId(externalCardId).FromAmount(fromAmount).FromCreationTime(fromCreationTime).FromLastUpdatedTime(fromLastUpdatedTime).Id(id).OriginalTransferId(originalTransferId).OriginatingAccountId(originatingAccountId).OriginatingCustomerId(originatingCustomerId).PaymentAccountReference(paymentAccountReference).Status(status).Tenant(tenant).ThreeDsId(threeDsId).ToAmount(toAmount).ToCreationTime(toCreationTime).ToLastUpdatedTime(toLastUpdatedTime).TransactionId(transactionId).Type_(type_).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.ListExternalCardTransfers``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListExternalCardTransfers`: TransferListResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.ListExternalCardTransfers`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiListExternalCardTransfersRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **limit** | **int32** |  | [default to 100]
 **pageToken** | **string** |  | 
 **accountId** | **[]string** |  | 
 **amount** | **[]int32** |  | 
 **customerId** | **[]string** |  | 
 **externalCardId** | **[]string** |  | 
 **fromAmount** | **int32** |  | 
 **fromCreationTime** | **time.Time** |  | 
 **fromLastUpdatedTime** | **time.Time** |  | 
 **id** | **[]string** |  | 
 **originalTransferId** | **[]string** |  | 
 **originatingAccountId** | **[]string** |  | 
 **originatingCustomerId** | **[]string** |  | 
 **paymentAccountReference** | **string** |  | 
 **status** | [**[]TransferStatus**](TransferStatus.md) |  | 
 **tenant** | **[]string** |  | 
 **threeDsId** | **[]string** |  | 
 **toAmount** | **int32** |  | 
 **toCreationTime** | **time.Time** |  | 
 **toLastUpdatedTime** | **time.Time** |  | 
 **transactionId** | **[]string** |  | 
 **type_** | [**[]TransferType**](TransferType.md) |  | 

### Return type

[**TransferListResponse**](TransferListResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## ListExternalCards

> ExternalCardListResponse ListExternalCards(ctx).Limit(limit).PageToken(pageToken).BusinessId(businessId).CustomerId(customerId).FromCreationTime(fromCreationTime).FromDeletionTime(fromDeletionTime).FromLastUpdatedTime(fromLastUpdatedTime).Id(id).LastFour(lastFour).Name(name).NameLike(nameLike).PaymentAccountReference(paymentAccountReference).Status(status).StatusNeq(statusNeq).Tenant(tenant).ToCreationTime(toCreationTime).ToDeletionTime(toDeletionTime).ToLastUpdatedTime(toLastUpdatedTime).Execute()

List External Cards



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
    "time"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	limit := int32(100) // int32 |  (optional) (default to 100)
	pageToken := "a8937a0d" // string |  (optional)
	businessId := []string{"Inner_example"} // []string |  (optional)
	customerId := []string{"Inner_example"} // []string |  (optional)
	fromCreationTime := time.Now() // time.Time |  (optional)
	fromDeletionTime := time.Now() // time.Time |  (optional)
	fromLastUpdatedTime := time.Now() // time.Time |  (optional)
	id := []string{"Inner_example"} // []string |  (optional)
	lastFour := []string{"Inner_example"} // []string |  (optional)
	name := []string{"Inner_example"} // []string |  (optional)
	nameLike := "nameLike_example" // string | Case sensitive wildcard search for name, wildcards can be specified with '*'. Wildcards at both the start and the end of the input is assumed. (optional)
	paymentAccountReference := []string{"Inner_example"} // []string |  (optional)
	status := []openapiclient.ExternalCardStatus{openapiclient.external_card_status("ACTIVE")} // []ExternalCardStatus |  (optional)
	statusNeq := openapiclient.external_card_status("ACTIVE") // ExternalCardStatus |  (optional)
	tenant := []string{"abcdef_ghijkl"} // []string |  (optional)
	toCreationTime := time.Now() // time.Time |  (optional)
	toDeletionTime := time.Now() // time.Time |  (optional)
	toLastUpdatedTime := time.Now() // time.Time |  (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.ListExternalCards(context.Background()).Limit(limit).PageToken(pageToken).BusinessId(businessId).CustomerId(customerId).FromCreationTime(fromCreationTime).FromDeletionTime(fromDeletionTime).FromLastUpdatedTime(fromLastUpdatedTime).Id(id).LastFour(lastFour).Name(name).NameLike(nameLike).PaymentAccountReference(paymentAccountReference).Status(status).StatusNeq(statusNeq).Tenant(tenant).ToCreationTime(toCreationTime).ToDeletionTime(toDeletionTime).ToLastUpdatedTime(toLastUpdatedTime).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.ListExternalCards``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListExternalCards`: ExternalCardListResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.ListExternalCards`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiListExternalCardsRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **limit** | **int32** |  | [default to 100]
 **pageToken** | **string** |  | 
 **businessId** | **[]string** |  | 
 **customerId** | **[]string** |  | 
 **fromCreationTime** | **time.Time** |  | 
 **fromDeletionTime** | **time.Time** |  | 
 **fromLastUpdatedTime** | **time.Time** |  | 
 **id** | **[]string** |  | 
 **lastFour** | **[]string** |  | 
 **name** | **[]string** |  | 
 **nameLike** | **string** | Case sensitive wildcard search for name, wildcards can be specified with &#39;*&#39;. Wildcards at both the start and the end of the input is assumed. | 
 **paymentAccountReference** | **[]string** |  | 
 **status** | [**[]ExternalCardStatus**](ExternalCardStatus.md) |  | 
 **statusNeq** | [**ExternalCardStatus**](ExternalCardStatus.md) |  | 
 **tenant** | **[]string** |  | 
 **toCreationTime** | **time.Time** |  | 
 **toDeletionTime** | **time.Time** |  | 
 **toLastUpdatedTime** | **time.Time** |  | 

### Return type

[**ExternalCardListResponse**](ExternalCardListResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## Lookup3DS

> Lookup3dsResponse Lookup3DS(ctx).Lookup3dsRequest(lookup3dsRequest).IdempotencyKey(idempotencyKey).Execute()

Lookup 3DS



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	lookup3dsRequest := openapiclient.lookup_3ds_request{Lookup3dsRequestBrowser: openapiclient.NewLookup3dsRequestBrowser("AuthenticationIndicator_example", "BROWSER", "7d943c51-e4ff-4e57-9558-08cab6b963c7", "TransactionMode_example")} // Lookup3dsRequest | Details of the External Card Transfer 3DS authentication lookup.
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.Lookup3DS(context.Background()).Lookup3dsRequest(lookup3dsRequest).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.Lookup3DS``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `Lookup3DS`: Lookup3dsResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.Lookup3DS`: %v\n", resp)
}
```

### Path Parameters



### Other Parameters

Other parameters are passed through a pointer to a apiLookup3DSRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **lookup3dsRequest** | [**Lookup3dsRequest**](Lookup3dsRequest.md) | Details of the External Card Transfer 3DS authentication lookup. | 
 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**Lookup3dsResponse**](Lookup3dsResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## RevealExternalCardDetails

> ExternalCardRevealResponse RevealExternalCardDetails(ctx, externalCardId).IdempotencyKey(idempotencyKey).Execute()

Reveal External Card Details



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	externalCardId := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | The unique identifier of an external card
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.RevealExternalCardDetails(context.Background(), externalCardId).IdempotencyKey(idempotencyKey).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.RevealExternalCardDetails``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `RevealExternalCardDetails`: ExternalCardRevealResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.RevealExternalCardDetails`: %v\n", resp)
}
```

### Path Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
**ctx** | **context.Context** | context for authentication, logging, cancellation, deadlines, tracing, etc.
**externalCardId** | **string** | The unique identifier of an external card | 

### Other Parameters

Other parameters are passed through a pointer to a apiRevealExternalCardDetailsRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------

 **idempotencyKey** | **string** | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. | 

### Return type

[**ExternalCardRevealResponse**](ExternalCardRevealResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)


## UpdateExternalCard

> ExternalCardResponse UpdateExternalCard(ctx, externalCardId).ExternalCardUpdateRequest(externalCardUpdateRequest).Execute()

Update External Card



### Example

```go
package main

import (
	"context"
	"fmt"
	"os"
	openapiclient "github.com/paybotic/synctera_client_library/client_v2"
)

func main() {
	externalCardId := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | The unique identifier of an external card
	externalCardUpdateRequest := *openapiclient.NewExternalCardUpdateRequest(openapiclient.external_card_status_request("ACTIVE")) // ExternalCardUpdateRequest | Fields to update

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalCardsAPI.UpdateExternalCard(context.Background(), externalCardId).ExternalCardUpdateRequest(externalCardUpdateRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExternalCardsAPI.UpdateExternalCard``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `UpdateExternalCard`: ExternalCardResponse
	fmt.Fprintf(os.Stdout, "Response from `ExternalCardsAPI.UpdateExternalCard`: %v\n", resp)
}
```

### Path Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
**ctx** | **context.Context** | context for authentication, logging, cancellation, deadlines, tracing, etc.
**externalCardId** | **string** | The unique identifier of an external card | 

### Other Parameters

Other parameters are passed through a pointer to a apiUpdateExternalCardRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------

 **externalCardUpdateRequest** | [**ExternalCardUpdateRequest**](ExternalCardUpdateRequest.md) | Fields to update | 

### Return type

[**ExternalCardResponse**](ExternalCardResponse.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: application/json
- **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints)
[[Back to Model list]](../README.md#documentation-for-models)
[[Back to README]](../README.md)

