...
> InternalAccountsList ListInternalAccounts(ctx).Limit(limit).PageToken(pageToken).SortBy(sortBy).Id(id).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.InternalAccountsAPI.ListInternalAccounts(context.Background()).Limit(limit).PageToken(pageToken).SortBy(sortBy).Id(id).Execute()
	if err != nil {
