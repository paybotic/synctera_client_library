...
func main() {
	addAccountsRequest := *openapiclient.NewAddAccountsRequest(*openapiclient.NewAddAccountsRequestAccountIdentifiers(), []string{"Jane Smith"}, openapiclient.ext_account_customer_type("PERSONAL"), *openapiclient.NewAddAccountsRequestRoutingIdentifiers([]string{"US"}, "Chase"), "Type_example") // AddAccountsRequest | 
...
func main() {
	externalAccountLinkToken := *openapiclient.NewExternalAccountLinkToken("ClientName_example", []string{"CountryCodes_example"}, openapiclient.external_account_language("EN"), "Type_example") // ExternalAccountLinkToken | 
	idempotencyKey := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | An idempotency key is an arbitrary unique value generated by client to detect subsequent retries of the same request. It is recommended that a UUID or a similar random identifier be used as an idempotency key. A different key must be used for each request, unless it is a retry. (optional)
...
> ExternalAccountsList ListExternalAccounts(ctx).CustomerId(customerId).BusinessId(businessId).Limit(limit).PageToken(pageToken).IncludeHistory(includeHistory).Id(id).Nickname(nickname).Execute()
...
func main() {
	...
	nickname := "nickname_example" // string | A user-meaningful name for the account.  (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExternalAccountsAPI.ListExternalAccounts(context.Background()).CustomerId(customerId).BusinessId(businessId).Limit(limit).PageToken(pageToken).IncludeHistory(includeHistory).Id(id).Nickname(nickname).Execute()
	if err != nil {
