...
func (a *WebhooksAPIService) ListWebhooksExecute(r ApiListWebhooksRequest) (*WebhookList, *http.Response, error) {
	...
	if r.isEnabledOnly != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "is_enabled_only", r.isEnabledOnly, "")
	}
	if r.limit != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "limit", r.limit, "")
	} else {
...
// Delay the event triggering in seconds. Events are checked once a minute, so a short delay may not result in an immedidate resend.
...
func (a *WebhooksAPIService) RevokeSecretExecute(r ApiRevokeSecretRequest) (*http.Response, error) {
	...
	if r.oldSecretOnly != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "old_secret_only", r.oldSecretOnly, "")
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}
