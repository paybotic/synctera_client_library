...
> IncomingAchList GetIncomingACHList(ctx).Status(status).SettlementDateTo(settlementDateTo).SettlementDateFrom(settlementDateFrom).AccountId(accountId).PageToken(pageToken).Limit(limit).Execute()
...
func main() {
	status := "status_example" // string | Status of the transaction (optional)
	...
	accountId := "7d943c51-e4ff-4e57-9558-08cab6b963c7" // string | uuid representing an account (optional)
	pageToken := "a8937a0d" // string |  (optional)
	limit := int32(100) // int32 |  (optional) (default to 100)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ACHAPI.GetIncomingACHList(context.Background()).Status(status).SettlementDateTo(settlementDateTo).SettlementDateFrom(settlementDateFrom).AccountId(accountId).PageToken(pageToken).Limit(limit).Execute()
	if err != nil {
 ...
 **accountId** | **string** | uuid representing an account | 
 **pageToken** | **string** |  | 
